...
describe("testimonial-card media urls", () => {
  ...
  it("renders play badge when mediaUrl is mp4", async () => {
    const { TestimonialCard } = await import("../components/ui/testimonial-card")
    const html = renderToStaticMarkup(
      React.createElement(TestimonialCard, {
        name: "A",
    ...
    expect(html).toContain('data-testid="play-badge"')
  })

  it("does not render play badge when mediaUrl is non-mp4", async () => {
    const { TestimonialCard } = await import("../components/ui/testimonial-card")
    const html = renderToStaticMarkup(
      React.createElement(TestimonialCard, {
        name: "A",
    ...
    expect(html).not.toContain('data-testid="play-badge"')
  })
})
