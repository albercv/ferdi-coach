...
function buildBankDetailsLines(details: BankDetails): string {
  const swiftLine = details.swift ? `<strong>SWIFT/BIC:</strong> ${details.swift}<br/>` : ""
  const bankNameLine = details.bankName ? `<strong>Banco:</strong> ${details.bankName}<br/>` : ""
  return `${bankNameLine}${swiftLine}<strong>IBAN:</strong> ${details.iban}<br/>`
}
...
export function buildConfirmacionPagoEmail(s: PaymentSubmission, bankDetails: BankDetails): EmailTemplate {
  return {
    ...
    html: buildWrapper(`
      ...
      ${accentBlock(`
        ${buildBankDetailsLines(bankDetails)}
...
export function buildRecordatorioPagoEmail(s: PaymentSubmission, bankDetails: BankDetails): EmailTemplate {
  return {
...
export function buildPagoFallidoEmail(s: PaymentSubmission, bankDetails: BankDetails): EmailTemplate {
  return {
